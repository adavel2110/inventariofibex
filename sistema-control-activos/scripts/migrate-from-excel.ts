  ...
  try {
    ...
    const sedeDefault = await prisma.sede.upsert({
      ...
      create: {
        ...
        ciudad: "Ciudad",
        estado: "ACTIVO",  // Agregado campo requerido
        activo: true,
      },
    })
    ...
    for (const row of data) {
      try {
          ...
          if (!empleadoId) {
            ...
            const empleado = await prisma.empleado.create({
              data: {
                ...
                cargo: "Empleado",
                sede: { connect: { id: sedeDefault.id } },  // Conectar por relación
                departamento: { connect: { id: deptDefault.id } },  // Conectar por relación
        ...
        const stockItem = await prisma.stockItem.create({
          data: {
            id: stockItemId,
            nombreEquipo: row["NOMBRE PC"] || null,  // Mapeado al campo correcto
        ...
        for (const perif of perifericos) {
          if (perif.nombre && perif.nombre.trim() !== "") {
            ...
            await prisma.stockItem.create({
              data: {
                id: uuidv4(),
                codigoInterno: `${perif.tipo}-${Date.now()}-${Math.random().toString(36).substr(2, 5)}`,
